...
class sail_cSim(pluginTemplate):
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        if "D" in ispec["ISA"]:
            self.isa += "d"
        objdump = "riscv{0}-unknown-elf-objdump".format(64)  # Piconut has 64 toolchain
        if shutil.which(objdump) is None:
            logger.error(
                objdump + ": executable not found. Please check environment setup."
            )
            raise SystemExit(1)
        compiler = "riscv{0}-unknown-elf-gcc".format(64)  # Piconut has 64 toolchain
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            cmd = (
                self.compile_cmd.format(
                    testentry["isa"].lower(), 64
                )  # Piconut has 64 toolchain
                + " "
                + test
                + " -o "
                + elf
            ...
            execute += self.objdump_cmd.format(
                elf, 64, "ref.disass"
            )  # Piconut has 64 toolchain
