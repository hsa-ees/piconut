...
void Tiny_Flip_TTRIS(uint8_t HR_TTRIS)
{
    ...
    // PHASE 1: Render to Buffer (CPU intensive, no I2C)
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < HR_TTRIS; x++)
        {
            frame_buffer[y][x] = Recupe_TTRIS(x, y);
        }
    }

    // PHASE 2: Send to OLED (Fast I2C transfer, optimized)
...
void Flip_intro_TTRIS(uint8_t* TIMER1)
{
    ...
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < 128; x++)
        {
            frame_buffer[y][x] = intro_TTRIS(x, y, TIMER1);
        }
    }

    // PHASE 2: Send to OLED
    JOY_OLED_update(frame_buffer);
}
