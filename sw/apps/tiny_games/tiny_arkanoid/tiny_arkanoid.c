...
//
// ===================================================================================
// Modified to work with PicoNut by Daniel Sommerfeldt: daniel.sommerfeldt1@tha.de
// ===================================================================================

#include "driver.h"
#include "spritebank.h"

uint8_t frame_buffer[8][128];
...
int main(void)
{
    ...
    NEXTLEVEL:
        ...
        JOY_sound(120, 100);
        JOY_sound(140, 100);
        if(VARIABLE.LEVELSPEED > 8)
            VARIABLE.LEVELSPEED = VARIABLE.LEVELSPEED - 2;
        Tiny_Flip(2, &VARIABLE);
        JOY_DLY_ms(400);
        ResetVar(&VARIABLE);
        VARIABLE.LEVEL++;
    ...
    ONE:
        ...
        while(1)
        {
            if(VARIABLE.Frame % 8 == 0)
            {
                ...
                if((VARIABLE.launch == 0) && (JOY_act_pressed()))
                    VARIABLE.launch = 1;
            ...
            if((VARIABLE.Frame % VARIABLE.LEVELSPEED == 0))
                UpdateBall(&VARIABLE);
            if(VARIABLE.Frame % 32 == 0)
                Tiny_Flip(0, &VARIABLE);
            if(VARIABLE.Frame == 48)
            {
                if(VARIABLE.ANIMREFLECT < 3)
                    VARIABLE.ANIMREFLECT++;
                if(BallMissing(&VARIABLE))
                    goto RESTARTLEVEL;
                if(CheckLevelEnded(&VARIABLE))
                    goto NEXTLEVEL;
            }
            if(VARIABLE.Frame < 64)
                VARIABLE.Frame++;
            else
                VARIABLE.Frame = 1;
            //JOY_SLOWDOWN();
...
uint8_t CheckLevelEnded(GROUPE* VAR)
{
    ...
    for(v = 0; v < 5; v++)
    {
        for(h = 0; h < 6; h++)
        {
            if((VAR->BlocsGrid[h][v] != 255) && (VAR->BlocsGrid[h][v] != 5))
            {
                res = 0;
            }
        }
...
uint8_t CheckCollisionBall(GROUPE* VAR)
{
    if(VAR->SIMBallxpos > 106)
    {
        return 1;
    }
    if(VAR->SIMBallypos > 59)
    {
        return 1;
    }
    if(VAR->SIMBallypos < 4)
    {
        return 1;
    }
    ...
    if(CheckCollisionWithBLOCK(VAR))
    {
        return 1;
    }
    return 0;
...
uint8_t CheckCollisionWithBLOCK(GROUPE* VAR)
{
    ...
    if(VAR->BlocsGrid[VAR->Py][VAR->Px] == 255)
    {
        return 0;
    }
    JOY_sound(150, 10);
...
uint8_t RecupeXPositionOnGrid(GROUPE* VAR)
{
    if((VAR->SIMBallxpos >= 66) && (VAR->SIMBallxpos < 72))
        return 0;
    else if((VAR->SIMBallxpos >= 72) && (VAR->SIMBallxpos < 78))
        return 1;
    else if((VAR->SIMBallxpos >= 78) && (VAR->SIMBallxpos < 84))
        return 2;
    else if((VAR->SIMBallxpos >= 84) && (VAR->SIMBallxpos < 90))
        return 3;
    else if((VAR->SIMBallxpos >= 90) && (VAR->SIMBallxpos < 96))
        return 4;
    return 255;
}
...
uint8_t RecupeYPositionOnGrid(GROUPE* VAR)
{
    if((VAR->SIMBallypos >= 8) && (VAR->SIMBallypos < 16))
        return 0;
    else if((VAR->SIMBallypos >= 16) && (VAR->SIMBallypos < 23))
        return 1;
    else if((VAR->SIMBallypos >= 23) && (VAR->SIMBallypos < 31))
        return 2;
    else if((VAR->SIMBallypos >= 31) && (VAR->SIMBallypos < 40))
        return 3;
    else if((VAR->SIMBallypos >= 40) && (VAR->SIMBallypos < 48))
        return 4;
    else if((VAR->SIMBallypos >= 48) && (VAR->SIMBallypos < 55))
        return 5;
    return 255;
}
...
uint8_t CheckCollisionWithTRACKBAR(GROUPE* VAR)
{
    ...
    if((VAR->SIMBallxpos > 6) || (VAR->SIMBallxpos < 5))
    {
        return 0;
    }
    if(TRACK > VAR->SIMBallypos)
    {
        return 0;
    }
    if((TRACK + 16) < VAR->SIMBallypos)
    {
        return 0;
    }
...
void Tiny_Flip(uint8_t render0_picture1, GROUPE* VAR)
{
    ...
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < 128; x++)
        {
            ...
            {
                frame_buffer[y][x] = (Block(x, y, VAR) | Ball(x, y, VAR) |
                                      TrackBar(x, y, VAR) | background(x, y) |
                                      PannelLive(x, y, VAR) | PannelLevel(x, y, VAR));
            }
            else if(render0_picture1 == 1)
            {
                frame_buffer[y][x] = (MAIN[x + (y * 128)]);
            }
            else if(render0_picture1 == 2)
            {
                frame_buffer[y][x] = background(x, y);
            }
        }
    }

    JOY_OLED_update(frame_buffer);
}

uint8_t PannelLevel(uint8_t X, uint8_t Y, GROUPE* VAR)
{
    if((Y < 5) || (Y > 6) || (X < 117) || (X > 123))
        return 0x00;
#define VAl10 (VAR->LEVEL / 10)
#define VAl01 (VAR->LEVEL - (VAl10 * 10))
    if(Y == 5)
    {
        return ((DIGITAL[(X - 117) + (VAl10 * 7)]));
    }
    else if(Y == 6)
    {
        return ((DIGITAL[(X - 117) + (VAl01 * 7)]));
    }
    return 0x00;
}
...
uint8_t Block(uint8_t X, uint8_t Y, GROUPE* VAR)
{
    ...
    if((X >= 67) && (X < 97) && (Y >= 1) && (Y <= 6))
    {
        ...
        if(VAR->BlocsGrid[(Y - 1)][XValue] == 255)
            return 0x00;
...
#define BALLYPOS (VAR->Ballypos - 1)
    ...
    if(VAR->BALLyDecal == 0)
    {
        if(Y == VAR->Ypos)
        {
            return ((BALL[(X - (uint8_t)(BALLXPOS))]));
        }
    }
    else
    {
        ...
        if(Y == VAR->Ypos)
        {
            return SplitSpriteDecalageY(DECAL, (BALL[(X - (uint8_t)(BALLXPOS))]), 1);
        }
        if(Y == (VAR->Ypos) + 1)
        {
            return SplitSpriteDecalageY(DECAL, (BALL[(X - (uint8_t)(BALLXPOS))]), 0);
        }
    }
    return 0x00;
}
...
uint8_t TrackBar(uint8_t X, uint8_t Y, GROUPE* VAR)
{
    ...
    if(X < 3)
        return 0;
    if(Y >= (3 + VAR->TrackBary))
        return 0;
    if(Y < (VAR->TrackBary))
        return 0;
    if(VAR->TrackBaryDecal == 0)
    {
        if(Y != VAR->TrackBary + 2)
        {
            return ((TRACKBAR[(X - 3) + ((Y - VAR->TrackBary) * 4)]));
        }
    }
    else
    {
        if(Y == VAR->TrackBary)
        {
            return SplitSpriteDecalageY(VAR->TrackBaryDecal, (TRACKBAR[(X - 3)]), 1);
        }
        else if(Y == VAR->TrackBary + 1)
        {
            return SplitSpriteDecalageY(VAR->TrackBaryDecal, (TRACKBAR[(X - 3)]), 0) | SplitSpriteDecalageY(VAR->TrackBaryDecal, (TRACKBAR[(X - 3) + 4]), 1);
        }
        else if((Y == VAR->TrackBary + 2) && (VAR->TrackBaryDecal != 0))
        {
            return SplitSpriteDecalageY(VAR->TrackBaryDecal, (TRACKBAR[(X - 3) + 4]), 0);
        }
    }
    return 0x00;
}
