...
void Tiny_Flip(uint8_t mode, GAME* game, DIGITAL* score, DIGITAL* velX, DIGITAL* velY)
{
    ...
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < 128; x++)
        {
            if(mode == 0)
            {
                frame_buffer[y][x] = (GameDisplay(x, y, game) | LivesDisplay(x, y, game) |
                                      DashboardDisplay(x, y, game) | ScoreDisplay(x, y, score) |
                                      VelocityDisplay(x, y, velX, 1) | VelocityDisplay(x, y, velY, 0) |
            ...
            {
                frame_buffer[y][x] = (StarsDisplay(x, y, game) | LivesDisplay(x, y, game) |
                                      DashboardDisplay(x, y, game) | ScoreDisplay(x, y, score) |
                                      VelocityDisplay(x, y, velX, 1) | VelocityDisplay(x, y, velY, 0) |
                                      FuelDisplay(x, y, game));
            }
