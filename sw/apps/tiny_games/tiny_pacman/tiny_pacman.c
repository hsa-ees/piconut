    ...
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < 128; x++)
        {
            if(render0_picture1 == 0)
            {
                if(INGAME)
                {
                    frame_buffer[y][x] = (background(x, y) | SpriteWrite(x, y, Sprite) | DotsWrite(x, y, Sprite) | LiveWrite(x, y) | FruitWrite(x, y));
                }
                else
                {
                    frame_buffer[y][x] = 0xff - (background(x, y) | SpriteWrite(x, y, Sprite));
                }
            }
            else if(render0_picture1 == 1)
            {
                frame_buffer[y][x] = (back[x + (y * 128)]);
            }
        }
    }
