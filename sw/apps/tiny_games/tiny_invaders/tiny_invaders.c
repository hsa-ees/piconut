...
void LoadMonstersLevels(int8_t Levels, SPACE* space)
{
    ...
    for(y = 0; y < 5; y++)
    {
        ...
        {
            if(y != 4)
                space->MonsterGrid[y][x] = MonstersLevels[(Levels * 24) + (y * 6) + x];
            else
    ...
    for(y = 0; y < 8; y++)
    {
        for(x = 0; x < 128; x++)
        {
            if(render0_picture1 == 0)
            {
                ...
                frame_buffer[y][x] = (background(x, y, space) | LivePrint(x, y) | Vesso(x, y, space) | UFOWrite(x, y, space) | Monster(x, y, space) | MyShoot(x, y, space) | MonsterShoot(x, y, space) | MYSHIELD);
            }
            else
            {
                frame_buffer[y][x] = intro[x + (y * 128)];
            }
        }
...
uint8_t Vesso(uint8_t x, uint8_t y, SPACE* space)
{
    ...
    {
        if(ShipDead == 0)
            return vesso[x - ShipPos];
        else
            return vesso[x - ShipPos + (12 * space->oneFrame)];
    }
    return 0;
}
    ...
    else
    {
        ...
        if(space->PositionDansGrilleMonsterY == 0)
        {
            ...
            if(SpriteType < 8)
                ...
                ANIMs = 0;
            if(SpriteType != -1)
                Murge2 = SplitSpriteDecalageY(Monsters[(WriteMonster14(x - space->MonsterGroupeXpos) + SpriteType * 14) + ANIMs], 1, space);
            else
                Murge2 = 0x00;
            return Murge2;
        }
        else
        {
            ...
            if(SpriteType < 8)
                ...
                ANIMs = 0;
            if(SpriteType != -1)
                Murge1 = SplitSpriteDecalageY(Monsters[(WriteMonster14(x - space->MonsterGroupeXpos) + SpriteType * 14) + ANIMs], 0, space);
            else
                Murge1 = 0x00;
            SpriteType = space->MonsterGrid[space->PositionDansGrilleMonsterY][space->PositionDansGrilleMonsterX];
            if(SpriteType < 8)
