...
```{figure} ./figures/uart/uart_overview.drawio.svg
:name: uart_overview
:width: 100%
:align: center
...
```{doxygenfunction} SC_MODULE(m_uart)
:project: m_uart
```

The Wishbone UART module consists of the following components:

- **[Baudgen](sec:uart:uart:baudgen)**: Multiple Baudgenerators that divide the system clock to generate the baudrate for the UART module.
- **[Majority Filter](sec:uart:uart:majr_filter)**: The majority filter is used to filter the received data. To ensure that the bit is received correctly.
- **[FIFOs](sec:uart:uart:fifo)**: The FIFOs are used to buffer the data that is sent and received by the UART module.
- **[uart_rx](sec:uart:uart:rx)**: The receiver receives data from the outside world.
- **[uart_tx](sec:uart:uart:tx)**: The transmitter sends data to the outside world.

(sec:uart:uart:baudgen)=

### Baudgen

```{figure} ./figures/uart/uart_baudgen.drawio.svg
:name: uart_baudgen
:width: 75%
:align: center
Diagram of the Baudrate Generator.
```
...
(sec:uart:uart:majr_filter)=

### Majority Filter

```{figure} ./figures/uart/uart_majr_filter.drawio.svg
:name: uart_majority_filter
:width: 75%
:align: center
Diagram of the Majority Filter.
```
...
```{figure} ./figures/uart/uart_fifo.drawio.svg
:name: uart_fifo
:width: 75%
:align: center
Diagram of the FIFOs.
```
...
```{figure} ./figures/uart/uart_rx.drawio.svg
:name: uart_rx
:width: 75%
:align: center
...
```{figure} ./figures/uart/uart_tx.drawio.svg
:name: uart_tx
:width: 75%
:align: center
...
- **[UART Echo](sec:uart_uart:system:uart_echo)**: Contains the test hardware for the UART module.
- **[Top](sec:uart_uart:system:top)**: Contains a wrapper for synthesis

(sec:uart_uart:system:uart_echo)=

#### UART Echo (m_uart_echo)

```{doxygenfunction} SC_MODULE(m_uart_echo)
:project: m_uart_system
```

(sec:uart_uart:system:top)=
...
The uart_soft module implements a UART module that can be connected to the `CSoftPeripheral` interface of the PicoNut processor, which is described in Chapter {ref}`sec:idef:csoft`.

The following figure provides an overview of the components of the uart_soft module.

```{figure} ./figures/uart/uart_soft.drawio.svg
:name: sim_uart_overview
:width: 100%
:align: center
Overview of the soft_uart module.
```
...
A C++ Vector is used to emulate the FIFO of the UART module and on each write to
the transmit register, the fifo is flushed.
