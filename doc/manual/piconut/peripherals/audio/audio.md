...
The **[audio](sec:audio_hardware_peripheral:modules:audio)** top module consists of the following components:

- **[audio_single](sec:audio_hardware_peripheral:modules:audio_single)**: Module generating one audio voice.
...
(sec:audio_hardware_peripheral:modules:audio)=

### audio

```{figure} ./figures/audio/audio.svg
:name: audio
:width: 80%
:align: center
Diagramm of the `audio` top module containing {math}`n` voices.
```
...
(sec:audio_hardware_peripheral:modules:audio_single)=
...
`unsigned int *regs = (unsigned int *)(PN_CFG_AUDIO_BASE_ADDRESS + 0x08 * v)`:
...
```c
#define VOICE(NUM) ((unsigned int *)(PN_CFG_AUDIO_BASE_ADDRESS + 0x08 * NUM))
