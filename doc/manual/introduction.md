...
The PicoNut processor itself is part of a larger system built around it. The nucleus and the membrana (Memory+Wishbone interface) make up the PicoNut processor.
The PicoNut processor, a System bus and any kind of memory make up the required minimal system.

```{figure} figures/piconut-structure.svg
:alt: Alt text
:align: center
...
and other devices via the membrana. Communication between the nucleus and the membrana is
done via the {ref}`sec:idef:ipdp`.

### The Membrana

The Membrana serves as the processor nucleus' interface to the wishbone bus. It handles all read and write
